...
对 `skills/` 下阶段 I–IV 的 8 个 Skill 组成的建模流水线做客观的质量评估，回答三个问题：
...
> **无参考案例（专家评审制）**：当领域没有成熟开源参考实现时，不设 A 类锚点，改用四类判据——业务完整性（简报要素全覆盖）、DDD 原则合规、内部一致性（跨 Skill 工件衔接）、正向越位识别（见 [insurance-validation/scoring/rubric.md](./insurance-validation/scoring/rubric.md)）。评审要求：**至少 2 名评审交叉评分，或业务方抽验 ≥ 3 条关键不变量**，缓解无真值时的主观性；分数注明"与有真值案例不可直接对比"。
...
- **专家评审制的主观性**：无参考案例（[insurance-validation](./insurance-validation/)）采用专家评审制，业务完整性判据无法外部验证；缓解方式：至少 2 名评审交叉评分，或业务方抽验 ≥ 3 条关键不变量（见 §4.2 说明）。
