...
**设计边界**：本体系仅覆盖**领域建模**（战略 + 战术），不涉及代码实现、测试策略或架构合规性检查。
...
体系主干采用 4 个阶段（Stage），代表 **4 种不同类型的建模工作**，而非严格的线性步骤：
...
| Skill              | 职责                       | 核心输出                                       |
| :----------------- | :------------------------- | :--------------------------------------------- |
| `ddd-model-review` | 全局模型质量评估与反馈闭环 | 一致性评分、完整性检查、耦合分析、回溯触发清单 |
...
本附录展示体系主干 8 个 Skill 在实际场景中的协作流程片段。
